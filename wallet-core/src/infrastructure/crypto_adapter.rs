...
impl CryptoProvider for CryptoAdapter {
    ...
    fn sign_evm_hash(&self, seed: &[u8], message_hash: [u8; 32]) -> Result<Vec<u8>, String> {
        let signing_key = SigningKey::from_slice(&seed[0..32]).map_err(|e| e.to_string())?;
...
impl CryptoAdapter {
    fn derive_evm_address(&self, seed: &[u8]) -> Result<String, String> {
        let signing_key = SigningKey::from_slice(&seed[0..32]).map_err(|e| e.to_string())?;
    ...
    fn derive_solana_address(&self, seed: &[u8]) -> Result<String, String> {
        let seed_bytes: [u8; 32] = seed[0..32]
            .try_into()
